...
public class DBMgr
{
    ...
    public void Init()
    {
        conn = new MySqlConnection("server=127.0.0.1;port=3306;database=darkgod;user=root;password=root;charset=utf8");
        conn.Open();
        PECommon.Log("DBMgr Init Done.");
        //QueryPlayerData("1111", "xxxx");
    }
            ...
            if (reader.Read())
            {
                ...
                if (_pass.Equals(pass))
                {
                    ...
                    for (int i = 0; i < taskStrArr.Length; i++)
                    {
                        ...
                        {
                            throw new System.Exception("DataError");
                        }
                    }
                    #endregion
    ...
    private int InsertNewAcctData(string acct, string pass, PlayerData pd)
    {
        ...
        try
        {
            MySqlCommand cmd = new MySqlCommand("insert into account set acct=@acct,pass=@pass,name=@name," +
                ...
                "pierce=@pierce,critical=@critical,guideid=@guideid,strong=@strong,time=@time,task=@task,fuben=@fuben", conn);
            cmd.Parameters.AddWithValue("acct", acct);
    ...
    public bool UpdatePlayerData(int id, PlayerData playerData)
    {
        try
        {
            MySqlCommand cmd = new MySqlCommand("update account set name=@name,level=@level,exp=@exp,power=@power," +
                "coin=@coin,diamond=@diamond,crystal=@crystal,hp=@hp,ad=@ad,ap=@ap,addef=@addef,apdef=@apdef,dodge=@dodge," +
                "pierce=@pierce,critical=@critical,guideid=@guideid,strong=@strong,time=@time,task=@task,fuben=@fuben where id=@id", conn);
